...
rn, what i am starting on is the integration of prose and racket-commonmark
...
then wraps all of the exprs in a runtime-wrapper / _prose-main  module
...
~/Projects/prose/prose-lib/main.rkt does the at-expr business.
- resultant module (~runtime-wrapper~)
  - itself has a module inside, configure-runtime (no idea what it does?)
    - does the display logic when module is run according to comment in
      ~/Projects/prose/prose-lib/private/configure-runtime.rkt
  - another module, _prose-main - obv the "main" runtime thing?
  - which is then required into runtime-wrapper (why?)
    - only brings in the show function??? show function used to "show" document
      when run as module

- what is prose-doc-id??? comes from constatnts, doc-id prefixed with prose-
